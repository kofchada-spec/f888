...
serve(async (req) => {
  ...
  try {
    ...
    // Get the subscription plan from request body
    ...
    const priceData = plan === 'monthly' 
      ? {
          currency: "eur",
          product_data: { name: "FitPaS - Abonnement Mensuel" },
          unit_amount: 999, // 9.99 EUR
          recurring: { interval: "month" as const },
        }
      : {
          currency: "eur",
          product_data: { name: "FitPaS - Abonnement Annuel (33% de remise)" },
          unit_amount: 7999, // 79.99 EUR
