...
serve(async (req) => {
  ...
  try {
    ...
    // Check if user is in free trial period (2 weeks)
    const now = new Date();
    const isInFreeTrial = existingSubscriber?.trial_end && new Date(existingSubscriber.trial_end) > now;

    logStep("Checking trial status", { 
      hasExistingRecord: !!existingSubscriber, 
    ...
    if (customers.data.length === 0) {
      logStep("No customer found, updating subscription state");
    ...
    if (hasActiveSub) {
      ...
      // Determine subscription tier from price (in cents)
      const priceId = subscription.items.data[0].price.id;
      const price = await stripe.prices.retrieve(priceId);
      const amount = price.unit_amount || 0;
      
      if (amount === 999) { // 9.99 EUR monthly
        subscriptionTier = "Monthly";
      } else if (amount === 7999) { // 79.99 EUR yearly
