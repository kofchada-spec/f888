...
const EnhancedMap: React.FC<EnhancedMapProps> = ({ 
  ...
  useEffect(() => {
    console.log('🔍 DIAGNOSTIC - Auto-generate conditions:', {
      hasMap: !!map.current,
      mapStyle: map.current?.isStyleLoaded?.() || 'no-map',
      hasPlanningData: !!planningData,
      planningDataValue: planningData,
      hasUserLocation: !!userLocation,
      userLocationValue: userLocation,
      hasMapboxToken: !!mapboxToken,
      mapboxTokenValue: mapboxToken,
      hasAutoGenerated,
      isCalculating
    });
    ...
    if (map.current && planningData && userLocation && mapboxToken && !hasAutoGenerated && !isCalculating) {
      const autoGenerate = async () => {
        console.log('🚀 Starting auto-generation for:', {
        ...
        try {
          if (planningData.tripType === 'round-trip') {
            console.log('📍 Generating round-trip route...');
            routeData = await generateRoundTripRoute();
          } else {
            console.log('📍 Generating one-way route...');
            routeData = await generateOneWayRoute();
          }

          console.log('✅ Route generation result:', routeData);
          ...
          if (routeData) {
            ...
            console.log('❌ No route data returned from generation');
          }
        } catch (error) {
