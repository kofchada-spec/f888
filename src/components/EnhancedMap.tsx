...
const EnhancedMap: React.FC<EnhancedMapProps> = ({ 
  ...
  const [firstRouteData, setFirstRouteData] = useState<any>(null); // Stocker le premier itinéraire
  const [isReset, setIsReset] = useState(false); // Mode réinitialisé (clics désactivés)
  ...
  const displayRoute = async (destinationCoords: [number, number], route: any, routeDistanceKm: number) => {
    ...
    // Sauvegarder le premier itinéraire généré automatiquement (pas de sélection manuelle)
    if (!isManualSelection && !firstRouteData) {
      setFirstRouteData(routeData);
      console.log('Premier itinéraire sauvegardé');
    }
  ...
  const displayRoundTripRoute = async (destinationCoords: [number, number], outboundRoute: any, returnRoute: any, totalDistanceKm: number) => {
    ...
    // Sauvegarder le premier itinéraire aller-retour généré automatiquement
    if (!isManualSelection && !firstRouteData) {
      setFirstRouteData(routeData);
      console.log('Premier itinéraire aller-retour sauvegardé');
    }
  ...
  const handleMapClick = async (e: mapboxgl.MapMouseEvent) => {
    // Désactiver les clics si on est en mode réinitialisé
  ...
  useEffect(() => {
    if (forceReset) {
      ...
      setRouteError(null);
      setIsReset(true); // Activer le mode réinitialisé (clics désactivés)
      
      // Restaurer le premier itinéraire au lieu de générer un nouveau
      if (firstRouteData) {
        console.log('Restauration du premier itinéraire');
        onRouteCalculated?.(firstRouteData);
        
        // Restaurer l'affichage visuel sur la carte
        if (map.current && userLocation) {
          ...
          destinationMarker.current = new mapboxgl.Marker({ color: '#ef4444' })
            .setLngLat([firstRouteData.endCoordinates.lng, firstRouteData.endCoordinates.lat])
            .addTo(map.current);
          
          // Restaurer la route sur la carte
          if (map.current?.getSource('route')) {
            map.current.removeLayer('route');
            map.current.removeSource('route');
          }
          
          // Gérer les différents types de routes (simple ou aller-retour)
          if (firstRouteData.routeGeoJSON.type === 'FeatureCollection') {
            // Route aller-retour
            map.current?.addSource('route', {
              type: 'geojson',
              data: firstRouteData.routeGeoJSON
            });
            ...
            map.current?.addSource('route', {
              ...
              data: {
                type: 'Feature',
                properties: {},
                geometry: firstRouteData.routeGeoJSON
              }
            });
          ...
          // Ajuster la vue
          const coordinates = firstRouteData.routeGeoJSON.type === 'FeatureCollection' 
            ? firstRouteData.routeGeoJSON.features.flatMap((f: any) => f.geometry.coordinates)
            : firstRouteData.routeGeoJSON.coordinates;
        ...
        // Si pas de premier itinéraire sauvegardé, générer un nouveau (fallback)
        console.log('Aucun premier itinéraire sauvegardé, génération d\'un nouveau');
  ...
  }, [forceReset, firstRouteData, planningData, onResetComplete, userLocation]);
  ...
  return (
    <div className={`w-full h-[500px] rounded-2xl shadow-lg relative ${className}`}>
      ...
      {/* Message pour mode réinitialisé */}
      {planningData && isReset && (
        <div className="absolute bottom-4 left-4 right-4 bg-amber-50/90 backdrop-blur-sm p-3 rounded-lg shadow-lg border border-amber-200">
          <p className="text-sm text-center text-amber-800">
            ✓ Premier itinéraire restauré • Clics désactivés après réinitialisation
          </p>
        </div>
      )}
