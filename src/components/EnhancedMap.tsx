...
const EnhancedMap: React.FC<EnhancedMapProps> = ({ 
    ...
    try {
      ...
      if (outboundRoute && outboundRoute.geometry && outboundRoute.geometry.coordinates) {
        ...
        for (const route of data.routes) {
          ...
          // If we found a route with ≤40% overlap (60% different), use it
          if (overlap <= 0.40) {
        ...
        // If no route meets 60% criteria, try waypoint routing
        if (minOverlap > 0.40) {
  ...
  useEffect(() => {
    if (forceReset) {
      ...
      setRouteError(null);
      setHasAutoGenerated(false); // Reset flag to allow regeneration
      
      // Regenerate the default route instead of restoring saved data
      if (planningData?.tripType === 'one-way') {
        generateRandomDestination();
      } else if (planningData?.tripType === 'round-trip') {
        generateRoundTripDestination();
  ...
  }, [forceReset, planningData, onResetComplete]);
